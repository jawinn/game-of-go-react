...
const getBoardClass = (gridSize: number) => {
  return styles[`board-${gridSize.toString()}x${gridSize.toString()}`];
};
...
const Board = (props: BoardProps) => {
  ...
  const [boardData] = useState(Array(boardSize * boardSize).fill(0));
  ...
  return (
    <ol className={getBoardClass(boardSize)} data-testid="Board">
      {renderPoints}
    </ol>
  );
};
